...
class TestLibraryRent(TransactionCase):
    ...
    def setUpClass(cls):
        ...
        cls.user = cls.env["res.partner"].create(
            {"name": "Jane Smith", "category_id": [(6, 0, [cls.test_category.id])]}
        )

        cls.author_robert = cls.env["library.author"].create({"name": "Robert Martin"})
    ...
    def test_rent_book_success(self):
        ...
        self.assertFalse(
            self.book.is_available, "Book should be marked as unavailable after rent."
        )
        self.assertEqual(
            self.book.current_renter_id,
            self.user,
            "Current renter should be set to the renting user.",
        )
    ...
    def test_return_book(self):
        ...
        self.assertTrue(
            self.book.is_available, "Book should be available after return."
        )
        self.assertFalse(
            self.book.current_renter_id,
            "Current renter should be cleared after return.",
        )
